...
class ApprisalFeedback(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, lines):
        ...
        sheet.write(1,8, 'Performance Period',bold)
        sheet.write(1,9, 'Status',bold)
        sheet.write(1,10, 'Grade Type',bold)
        sheet.write(1,11, 'Employee Type',bold)
        sheet.write(1,12, 'Job Position',bold)
        sheet.write(1,13, 'Grade',bold)
        sheet.write(1,14, 'Mid Year Review Date',bold)
        sheet.write(1,15, 'End Year Review Date',bold)
        sheet.write(1,16, 'Company',bold)
        ...
        for line in lines: 
            ...
            sheet.write(row, 2, line.name.date.strftime('%d-%b-%Y') if line.name.date else '-', format1)
            sheet.write(row, 3, line.name.parent_id.name, format1)
            sheet.write(row, 4, line.name.department_id.manager_id.name if line.name.department_id.manager_id else '-', format1)
            sheet.write(row, 5, 'Active' if line.name.active==True else 'In-Active', format1)
            sheet.write(row, 6, line.name.work_location_id.name, format1)
            sheet.write(row, 7, line.name.department_id.name, format1)
            sheet.write(row, 8, str(financial_year), format1)  
            sheet.write(row, 9, str(appraisal_status), format1)
            sheet.write(row, 10, line.name.grade_type.name, format1) 
            sheet.write(row, 11,  str(employee_type), format1)  
            sheet.write(row, 12, line.name.job_id.name, format1)
            sheet.write(row, 13, line.name.grade_designation.name, format1) 
            sheet.write(row, 14, line.mid_year_date.strftime('%d-%b-%Y') if line.mid_year_date else '-', format1) 
            sheet.write(row, 15, line.end_year_date.strftime('%d-%b-%Y') if line.end_year_date else '-', format1) 
            sheet.write(row, 16, line.name.company_id.name, format1)
            row += 1           