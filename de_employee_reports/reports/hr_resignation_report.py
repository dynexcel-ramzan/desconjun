...
class EmployeeResignation(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, lines):
        ...
        sheet.write(1,13, 'DOB' ,bold)
        sheet.write(1,14, 'CNIC' ,bold)
        sheet.write(1,15, 'Company' ,bold)
        row = 2
        record_count=0
        for line in employees:
            ...
            sheet.write(row, 13, str(line.birthday), format1) 
            sheet.write(row, 14, str(line.cnic), format1)  
            sheet.write(row, 15, str(line.company_id.name), format1)
            row += 1 
            record_count += 1