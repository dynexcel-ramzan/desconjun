...
def expense_page_content(flag = 0, expense=0, categ=0, exception=0, subordinate=0, employee=0, warning=0, forcasted=0, acounting_date=0):
    ...
    if not contract:
        contract = request.env['hr.contract'].sudo().search([('employee_id','=', employees.id)], limit=1)
    cost_center_count = 0
    ...
    if categ !=0:
        ...
        unit_ora_categ = 0
        for exp_sub_line in products:
            unit_ora_categ = exp_sub_line.ora_unit
            break 
        if  unit_ora_categ=='km':
            meter_aplicable_list = []
            for meter_line in employees.vehicle_meter_line_ids:
                if meter_line.opening_reading > 0:
                    meter_aplicable_list.append(meter_line.sub_category_id.id)    
            products = request.env['expense.sub.category'].sudo().search([('ora_category_id','=', categ),('id','in', meter_aplicable_list)])
    ...
    if exception==0:
        ...
        products = request.env['expense.sub.category'].sudo().search([('id','in', products.ids)]) 
    controllers = request.env['controlled.account'].sudo().search([('id','in', expense_account_head)]) 
    error_flag = '0'
    errora_message = ''
    if warning !=0:
...
class CreateApproval(http.Controller):
    ...
    def expense_claim_create_template(self, **kw):
        ...
        contract = request.env['hr.contract'].sudo().search([('employee_id','=', employee),('state','=','open')], limit=1)
        if not contract:
            contract = request.env['hr.contract'].sudo().search([('employee_id','=', employee)], limit=1)    
        cost_center_count = 0
        default_controlled_account = 0
        cost_center_list = []
        cost_center_code_list = []
        ...
        for cost_center in contract.cost_center_information_line:
            ...
            if cost_center.by_default==True:
                default_expense_analytic = cost_center.cost_center.id
            cost_center_count += 1
        if  default_controlled_account==0:   
            ...
            return request.render("de_portal_expence.expense_req_validation", action_check_expense_warning(warning=warning_message))
        if  default_controlled_account==0:   
            warning_message='You are not allow to Submit Expense Claim! Your Cost Center is not set. Please contact with HR Department.'
    ...
    def create_expenses(self, **kw):
        ...
        product = request.env['expense.sub.category'].sudo().search([('id','=',int(kw.get('product_id')))], limit=1)            
        ...
        if kw.get('member_id'): 
            family_member = request.env['hr.employee.family'].sudo().search([('id','=', int(kw.get('member_id')))], limit=1)   
            if family_member:
                forcasted_data.update({
                    'member_id': family_member.id,
                })
        if kw.get('fleet_id') : 
            fleet = request.env['vehicle.meter.detail'].sudo().search([('id','=',int(kw.get('fleet_id')))], limit=1)           
            if fleet:
                forcasted_data.update({
                    'fleet_id': fleet.id,
                }) 
        ...
        if product.monthly_limit==True:
            ...
            monthly_expenses = request.env['hr.expense'].sudo().search(
        ...
        if employee.emp_type !='permanent':
            contract_sal = request.env['hr.contract'].sudo().search([ ('employee_id','=', employee.id),('state' ,'=', 'open') ])
            if contract_sal:
                limit = (contract_sal.wage/100) * employee.company_id.expense_percent
            if employee.company_id.expense_amt_limit > limit:
                limit = employee.company_id.expense_amt_limit
        ...
        if flag == False and exception!=True and product.ora_category_id.is_amount_limit==True:
            ...
            employee_expenses = request.env['hr.expense'].search(
                [('sub_category_id', '=', product.id), ('employee_id', '=', employee.id)
                    , ('state', '!=', 'refused'),('sheet_id.exception', '!=' ,True),
                 ('payment_mode', '=', 'own_account')])
            sum = 0 

            expense_subcateg_limit = request.env['hr.expense'].sudo().search(
                [('sub_category_id.parent_id', '=', product.id), ('employee_id', '=', employee.id)
                    , ('state', '!=', 'refused'),('sheet_id.exception', '!=' ,True),
                 ('payment_mode', '=', 'own_account')]) 
                
            for expense_subcateg in expense_subcateg_limit:
                if (expense_subcateg.create_date.date() > expense_period_date and expense_subcateg.create_date.date() <= fields.date.today()):
                    sum = sum + expense_subcateg.total_amount
            
            if product.parent_id: 
                employee_expenses_parent = request.env['hr.expense'].sudo().search(
    ...
    def create_expenses_lines(self, **kw):
        ...
        product = request.env['expense.sub.category'].sudo().search([('id','=',int(kw.get('product_id')))], limit=1)
        default_cost_center = int(kw.get('default_cost_center'))
        controller = request.env['product.product'].sudo().search([('sub_category_id','=',product.id),('ora_category_id','=', ora_category.id),('controlled_id','=',int(kw.get('controll_id')))], limit=1)
        cost_center_count = 0
        contract = request.env['hr.contract'].sudo().search([('employee_id','=', expense_sheet.employee_id.id),('state','=','open')], limit=1)
        if not contract:
            contract = request.env['hr.contract'].sudo().search([('employee_id','=', expense_sheet.employee_id.id)], limit=1)
        forcasted_data = {
            'expense_type': product.id,
            'reference':  kw.get('reference'),
            'unit_amount': kw.get('unit_amount'),
        ...
        for rec in employee.grade_designation.grade_line_ids:
            if   product.parent_id:
                ...
                period = int(rec.period)    
        expense_period_date = fields.date.today() - relativedelta(years=period)
        if expense_sheet.employee_id.emp_type !='permanent':
            contract_sal = request.env['hr.contract'].sudo().search([ ('employee_id','=', expense_sheet.employee_id.id),('state' ,'=', 'open') ])
            if contract_sal:
                limit = (contract_sal.wage/100) * expense_sheet.employee_id.company_id.expense_percent
            if expense_sheet.employee_id.company_id.expense_amt_limit > limit:
                limit = expense_sheet.employee_id.company_id.expense_amt_limit 
        ...
        if flag == False and expense_sheet.exception!=True and product.ora_category_id.is_amount_limit==True:
            ...
            employee_expenses = request.env['hr.expense'].sudo().search(
                [('sub_category_id', '=', product.id), ('employee_id', '=', employee.id)
                    , ('state', '!=', 'refused'),('sheet_id.exception', '!=' ,True),
                 ('payment_mode', '=', 'own_account')])
            sum = 0
            expense_sub_categ_limit = request.env['hr.expense'].sudo().search(
                [('sub_category_id.parent_id', '=', product.id), ('employee_id', '=', employee.id)
                    , ('state', '!=', 'refused'),('sheet_id.exception', '!=' ,True),
                 ('payment_mode', '=', 'own_account')]) 
            for expense_sub_categ in expense_sub_categ_limit:                    
                if (expense_sub_categ.create_date.date() > expense_period_date and expense_sub_categ.create_date.date() <= fields.date.today()):
                    sum = sum + expense_sub_categ.total_amount
