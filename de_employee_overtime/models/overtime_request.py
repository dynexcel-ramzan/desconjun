...
class HrOverTime(models.Model):
        ...
        for line in self:
            ...
            for ovt_type_line in line.overtime_type_id.type_line_ids:
                if ovt_type_line.compansation == 'leave':
                    if (shift.hours_per_day-1.5) <= line.hours and ovt_type_line.leave_type=='full_day':
                        leave_type = ovt_type_line.leave_type_id.id     
                        leave_period = ovt_type_line.leave_type
                    elif (shift.hours_per_day/2) <= line.hours and ovt_type_line.leave_type=='half_day':
                        leave_type = ovt_type_line.leave_type_id.id     
                        leave_period = ovt_type_line.leave_type
    ...
    def action_submit(self):
        for line in self:
            if line.state == 'draft':
                if line.overtime_hours > 4 and not line.remarks:
                ...
                if not line.overtime_type_id:
                    raise UserError('Not Allow to submit Without Overtime Type! Please Select Overtime Type Field.')
                line.update({
                    'state': 'to_approve'
                })
    ...
    def unlink(self):
        for ovt in self:
            if ovt.state not in ('draft','close'):
                ...
                att_exist = self.env['hr.attendance'].search([('employee_id','=',ovt.employee_id.id),('att_date','=',ovt.date)])
                if att_exist:
                    for att in att_exist:
                        att.update({
                            'is_overtime': False,
                        })
                ...
                if line.overtime_hours >= compansation.ot_hours:
                    if compansation.compansation == 'payroll':
                        if compansation.rate_type == 'fix_amount':
                            ...
                            ot_hour_amount = (((contract.wage/100) * compansation.percent_salary) /(compansation.hours_per_day * compansation.month_day))* compansation.rate_percent
    ...
    def generate_overtime_compansation(self):
        ...
        for line in self:
            ...
            if double_rate_line and single_rate_line:
                ...
                double_ot_hour_amount = (((contract.wage/100) * double_rate_line.percent_salary) /(double_rate_line.hours_per_day * double_rate_line.month_day)) *  double_rate_line.rate_percent 
                single_ot_hour_amount = (((contract.wage/100) * single_rate_line.percent_salary) /(single_rate_line.hours_per_day * single_rate_line.month_day)) * single_rate_line.rate_percent
