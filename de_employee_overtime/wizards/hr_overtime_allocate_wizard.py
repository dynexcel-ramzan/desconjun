...
class HrOvertimeAllocate(models.TransientModel):
    ...
    def _get_qualify_employees(self):
        work_locations = self.env['hr.work.location'].sudo().search([('admin_id.user_id','=',self.env.user.id)])
        qualify_employees = self.env['hr.employee'].search([('allow_overtime','=',True),('work_location_id','in', work_locations.ids)])
        return qualify_employees
    
    
    date_start = fields.Date(string='Date From', required=True, default=(fields.date.today().replace(day=1)-timedelta(1)).replace(day=16))
    date_end = fields.Date(string='Date To', required=True, default=fields.date.today().replace(day=15))
    employee_ids = fields.Many2many('hr.employee', string='Employees', domain=[('allow_overtime','=',True)])
    ...
    def get_normal_overtime_type(self, employee_company, work_location):
        ...
        if work_location:
            ...
            if not overtime_type:
                overtime_type = self.env['hr.overtime.type'].search([('type','=','normal'),('company_id','=',employee_company),('work_location_id','=',False)], limit=1)
    ...
    def get_gazetted_overtime_type(self, employee_company, work_location):
        ...
        if work_location:
            ...
            if not overtime_type:
                overtime_type = self.env['hr.overtime.type'].search([('type','=','gazetted'),('company_id','=',employee_company),('work_location_id','=',False)], limit=1)                
        return overtime_type
    ...
    def get_rest_days_overtime_type(self, employee_company, work_location):
        ...
        if work_location:
            ...
            if not overtime_type:
                overtime_type = self.env['hr.overtime.type'].search([('type','=','rest_day'),('company_id','=',employee_company),('work_location_id','=',False)], limit=1)                
    ...
    def action_create_overtime(self):
        ...
        for att in attendances:
            ...
            overtime_limit = 0
            daily_total_ovt_hrs = 0
            duplicate_attendance = self.env['hr.attendance'].search([('employee_id','=',att.employee_id.id),('att_date','=',att.att_date),('id','!=',att.id)])
            for duplicate_att in duplicate_attendance:
                daily_total_ovt_hrs += duplicate_att.rounded_hours
                duplicate_att.update({ 'is_overtime': True })    
            if att.shift_id:
                overtime_limit = (att.rounded_hours+daily_total_ovt_hrs) - att.shift_id.hours_per_day
            else:
                overtime_limit = (att.rounded_hours+daily_total_ovt_hrs) - 8
            if att.is_overtime==True:
                overtime_limit = 0
            overtime_type = self.get_normal_overtime_type(att.employee_id.company_id.id, att.employee_id.work_location_id.id)
            shift_schedule_lines = self.env['hr.shift.schedule.line'].search([('employee_id','=', att.employee_id.id),('rest_day','=',True),('date','=',att.att_date),('state','=','posted')])
