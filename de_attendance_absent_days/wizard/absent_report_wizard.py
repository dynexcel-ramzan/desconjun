...
class AbsentReport(models.TransientModel):
    ...
    resign = fields.Boolean(string='Only Resigned')
    employee_ids = fields.Many2many('hr.employee', string='Employee')
    
    def print_report(self):
        data = {}
        data['form'] = self.read(['date_from', 'date_to','employee_ids','resign'])[0]
        return self._print_report(data)

    def _print_report(self, data):
        data['form'].update(self.read(['date_from', 'date_to','employee_ids', 'resign'])[0])
