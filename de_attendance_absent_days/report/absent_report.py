...
class EmployeeReportXlS(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, lines):
        ...
        sheet.write(4,1 , 'Company',header_row_style)
        sheet.write(4,2 , 'Work Location',header_row_style)
        sheet.write(4,3 , 'Employee name',header_row_style)
        sheet.write(4,4 , 'Employee code',header_row_style) 
        sheet.write(4,5 , 'Absent on',header_row_style)
        ...
        for line in absent_list:
            ...
            sheet.write(row, 1, line['company'], format2) 
            sheet.write(row, 2, line['location'], format2)
            sheet.write(row, 3, line['employee'], format2)
            sheet.write(row, 4, line['emp_code'], format2)            
            sheet.write(row, 5, line['absent_on'].strftime('%d-%b-%y'), format2)            
