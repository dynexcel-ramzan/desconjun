        ...
        for payslip in self:            
            ...
            for dayline in range(delta_days):
                ...
                for leave_day in leaves:
                    ...
                    leave_total_days += leave_day.number_of_days
                check_in_time = ''
                check_out_time = ''
            ...
            if attendance_day_count >= 0:
                ...
                if not work_entry_type:
                    ...
                    work_entry_type = self.env['hr.work.entry.type'].create(vals)
            ...
            if rest_day_count >= 0:
                work_entry_type = self.env['hr.work.entry.type'].search([('code','=','REST100')], limit=1)
                if not work_entry_type:
                    vals = {
            ...
            if leave_day_count >= 0:
                work_entry_type = self.env['hr.work.entry.type'].search([('code','=','LEAVE100')], limit=1)
                if not work_entry_type:
                    vals = {
                        'name': 'Leave Days',
                        'code': 'LEAVE100',
            ...
            if absent_day_count >= 0:
                work_entry_type = self.env['hr.work.entry.type'].search([('code','=','ABSENT100')], limit=1)
                if not work_entry_type:
                    vals = {
                        'name': 'Absent Days',
                        'code': 'ABSENT100',
            ...
            if  payslip.employee_id.date:
                date_from_joiner = payslip.date_to.replace(day=1)
                month_ext_days = self.env['fiscal.year.month'].search([('id','=',payslip.date_to.month)], limit=1).days
                date_to_joiner = payslip.date_to.replace(day=month_ext_days)
                if str(payslip.employee_id.date) > str(date_from_joiner)  and str(payslip.employee_id.date) < str(date_to_joiner):
                    delta_days = 0
                    tot_day_unpaid = 0
                    delta_days = ((date_to_joiner - payslip.employee_id.date)).days + 1 
                    ...
                    for workday in payslip.worked_days_line_ids:
                        if workday.work_entry_type_id.code=='ABSENT100':
                            days = workday.number_of_days
                            workday.update({
                               'number_of_days': tot_day_unpaid
                            })
                    for workday in payslip.worked_days_line_ids:    
                        if workday.work_entry_type_id.code=='WORK100':
                            total_days = days + workday.number_of_days
                            workday.update({
